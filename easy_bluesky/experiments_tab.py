...
class ExperimentsTab(QWidget):
    ...
    def new_experiment(self):
        ...
        exp_info    = {"name": name, "created": ts.isoformat(),
                       "description": ""}
        (exp_dir / "experiment.json").write_text(
            json.dumps(exp_info, indent=2))

        active_info = {"name": name, "path": str(exp_dir),
    ...
    def open_experiment(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Could not read experiment.json:\n{e}")
            return
    ...
    def update_history(self, items: list):
        ...
        for item in items:
            ...
            # Ground truth: only log if this run's JSONL file is in our runs/
            if run_uids:
                if not self._run_file_exists(runs_dir, run_uids):
                    self._logged_uids.add(uid)   # won't belong here; mark seen
                    continue
            else:
                # No run UIDs (failed before start?) — fall back to timestamp
