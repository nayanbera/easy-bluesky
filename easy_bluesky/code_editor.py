...
class CodeEditor(QPlainTextEdit):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        # Tab — indent all selected lines, or insert spaces at cursor
        ...
        # Shift+Tab — dedent all selected lines (or current line)
    ...
    def _is_multiline_selection(self) -> bool:
        cursor = self.textCursor()
    ...
    def _indent_selection(self, dedent: bool):
        ...
        start_block = doc.findBlock(sel_start)
        end_block   = doc.findBlock(sel_end)
        # Don't include a trailing block whose selection ends at column 0
        if sel_end == end_block.position() and end_block != start_block:
            end_block = end_block.previous()
        cursor.beginEditBlock()
        b = start_block
